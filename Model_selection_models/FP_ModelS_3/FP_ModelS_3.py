...
np.random.seed(1)
# os.chdir('.')
out_folder='./output_3e'
if not os.path.exists(out_folder):
    os.makedirs(out_folder)

var_cons=np.loadtxt('./parameters_FP_ModelS_3e.csv', delimiter=',', skiprows=1, usecols=[1,2])
...
t_well = np.loadtxt('FloPy_Test4_monitoring_wells.csv',delimiter=',')
        ...
        # Get the variables for this model
            ...
            for row_col in range(0, N):
                ...
                if row_col != 0 and row_col != N - 1:
                    ...
                    if hv > 0:
                        chd_rec.append(((layer, N - 1, row_col), hv))
            ...
            wel_rec = [[Nlay - 1, int(N / 4), int(N / 4), q],[Nlay - 1, int(N / 4), int(N *3 / 4), q]]
            ...
            # create evapotranspiration (evta) package
            ...
            ## Run simulation
            try:
                sim_tru = sim.run_simulation(silent=True)
            except:
            ...
            np.save(os.path.join(out_folder, 'well_dif'+str(run)+str(block).zfill(3)+'.npy'),well_dif)
            np.save(os.path.join(out_folder, 'm_well'+str(run)+str(block).zfill(3)+'.npy'),m_well)
...
# Metropolis-Hastings sampling on likelihood matrix
    ...
    apost=np.zeros(shape=[nu,blocksize], dtype=float)
